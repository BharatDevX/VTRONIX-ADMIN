...
export default function FollowUpsPage() {
  ...
  const isDelayed=(r: AdminFollowUpRecord) => r.delay_days>0;
  const filtered=useMemo(()=>records.filter(r=>filter==="all"?true:filter==="pending"?(!r.completed&&!isDelayed(r)):filter==="completed"?(r.completed&&!isDelayed(r)):isDelayed(r)),[records,filter]);
  ...
  return <div className="space-y-6 p-5 lg:p-6">
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><CalendarClock className="size-5"/>Follow-Up Records</CardTitle>
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1200px] text-sm"><thead><tr className="border-b text-left text-xs uppercase tracking-wide text-slate-500">
        ...
        <td className="px-3 py-4">{r.completed?<Badge tone="success"><CheckCircle2 className="mr-1 size-3"/>Completed</Badge>:isDelayed(r)?<Badge tone="danger"><Clock3 className="mr-1 size-3"/>Delayed</Badge>:<Badge tone="warning"><Clock3 className="mr-1 size-3"/>Pending</Badge>}</td>
        <td className="px-3 py-4">{r.completed_visit_date??"—"}</td><td className={`px-3 py-4 font-semibold ${r.delay_days>0?"text-rose-600":"text-emerald-600"}`}>{r.completed?(r.delay_days===0?"On time":`${r.delay_days} day(s) late`):(r.delay_days>0?`${r.delay_days} day(s) overdue`:"—")}</td><td className="px-3 py-4 text-slate-500">{r.location||"—"}</td><td className="max-w-[280px] px-3 py-4 text-slate-500">{r.discussion||"—"}</td><td className="max-w-[280px] px-3 py-4 text-slate-500">{r.outcome||"—"}</td>
      </tr>)}</tbody></table></div>}
    </CardContent></Card>
  </div>;
}
