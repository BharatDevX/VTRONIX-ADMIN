  ...
  export default function SalesPage() {
    ...
    const columns = useMemo<ColumnDef<SalesRecord>[]>(
      () => [
        ...
        { accessorKey: "box_count", header: "Box" },
    ...
    return (
      <>
        <OperationalPage
          ...
          rowsForExport={(records) => records.map((record) => ({ amount: record.amount, channel: record.channel, customer: record.customer_name, date: record.sale_date, dealer: record.dealer_name, employee: record.employee_name, product: record.product_name, pack_size: record.pack_size ?? "", box_count: record.box_count ?? "", quantity: record.quantity, rate: record.rate, retailer: record.retailer_name ?? "", farmer: record.farmer_name ?? "" }))}
